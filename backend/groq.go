...
type groqCompletionParams struct {
...
func groqConfigured(cfg *Config) bool {
	return cfg.GroqAPIKey != ""
}

func groqChat(cfg *Config, params groqCompletionParams) (string, error) {
...
func groqTranscribe(cfg *Config, data []byte, filename string) (string, error) {
...
func analyzeImageDeterministic(filename string) map[string]interface{} {
	...
	for _, p := range patterns {
		matched, _ := regexpMatch(p.re, lower)
		if matched {
			category = p.category
			break
		}
