...
func entranceDeltas(entrance EntranceEvidence) (float64, []AccessFactor) {
	...
	if entrance.HasRamp && entrance.Steps <= 2 {
		return 0.5, []AccessFactor{{Label: entrance.Name + ": " + formatInt(entrance.Steps) + " anak tangga dengan ramp", Kind: "neutral"}}
	}
	return -1, []AccessFactor{{Label: entrance.Name + ": " + formatInt(entrance.Steps) + " anak tangga tanpa ramp", Kind: "negative"}}
}
...
func entranceWidthFactor(entrance EntranceEvidence) []AccessFactor {
	...
	if *entrance.WidthCm >= 120 {
		return []AccessFactor{{Label: entrance.Name + ": pintu aksesibel " + formatInt(*entrance.WidthCm) + " cm", Kind: "positive"}}
	}
	return []AccessFactor{{Label: entrance.Name + ": pintu sempit " + formatInt(*entrance.WidthCm) + " cm", Kind: "negative"}}
}
	...
	},
	{
		...
		adjust: func(f FeatureEvidence) (float64, []AccessFactor) {
			...
			if numberOrNull(f.StepCount) != nil {
				label = "Tangga " + formatInt(*f.StepCount) + " anak tangga"
			}
