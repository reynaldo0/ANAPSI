...
func loadAccountStore(reporterId string) (AccountStore, error) {
	...
	row := db.QueryRow(`SELECT reporterName, reports, verifiedReports, points, badges FROM gamification WHERE reporterKey = ?`, reporterId)
	var name sqlNullableString
...
func saveAccountStore(reporterId string, account AccountStore) error {
	...
	_, err := db.Exec(
		...
		reporterId, name, account.Reports, account.VerifiedReports, account.Points, string(badgesJSON), nowMySQL(),
	)
	return err
}
...
func awardReportPoints(reporterId string, reporterName *string, input AwardInput) (GamificationStats, error) {
	...
	if err != nil {
		account = AccountStore{ReporterName: ""}
	}
...
func listGamificationRanking(limit int) []map[string]interface{} {
	rows, err := db.Query(`SELECT reporterName, reports, points FROM gamification ORDER BY points DESC LIMIT ?`, limit)
	if err != nil {
