...
func projectOnSegment(p, a, b LatLng) (nearest, along float64) {
	abx := b.Lng - a.Lng
	aby := b.Lat - a.Lat
	apsx := p.Lng - a.Lng
	apsy := p.Lat - a.Lat
	...
	proj := LatLng{Lng: a.Lng + t*abx, Lat: a.Lat + t*aby}
	return metersBetween(p, proj), t * metersBetween(a, b)
}

func subdivide(a, b LatLng, maxLegM float64) []LatLng {
	total := metersBetween(a, b)
	count := int(total / maxLegM)
	if count < 1 {
		count = 1
	}
	points := make([]LatLng, 0, count+1)
	for i := 0; i < count; i++ {
		t := float64(i) / float64(count)
		points = append(points, LatLng{Lat: a.Lat + (b.Lat-a.Lat)*t, Lng: a.Lng + (b.Lng-a.Lng)*t})
	}
...
func corridorHits(points []LatLng, kinds []string) []corridorHit {
	...
	total := cum
	for _, feature := range demoFeatureData() {
		...
		cumLeg := 0.0
		for i := 0; i < len(points)-1; i++ {
			nearest, along := projectOnSegment(LatLng{Lat: feature.Lat, Lng: feature.Lng}, points[i], points[i+1])
...
func sumPath(points []LatLng) float64 {
	total := 0.0
	for i := 0; i < len(points)-1; i++ {
...
func destPointAt(a, b LatLng, t float64) LatLng {
...
func perpendicularOffsetFastest(a, b, at LatLng, offsetMeters float64) LatLng {
	dx := b.Lng - a.Lng
	dy := b.Lat - a.Lat
	lenD := dx*dx + dy*dy
	if lenD == 0 {
...
func evaluateRoute(
	...
	for _, hit := range route.hits {
		meta := statusMeta(hit.feature.Kind, hit.feature.Status)
	...
	var recommendedEntrance *DemoEntrance
	...
	for _, strokeItem := range strokes {
		...
		meta := statusMeta(strokeItem.Kind, strokeItem.Status)
		if meta.Tone == "success" {
			facilitiesOf = append(facilitiesOf, RouteFacilityInfo{
				...
				Label:          LAYERS[strokeItem.Kind].Label + ": " + meta.Label,
				Symbol:         meta.Symbol,
				Tone:           "success",
...
func buildSteps(pivots []LatLng, destinationName string) []RouteStepInfo {
...
func findDemoPlace(id string) *DemoPlace {
...
func toOption(input routeOptionInput) RouteOption {
	...
	return RouteOption{
		...
		Steps:              input.Steps,
		HonestNote:         strPtr(honestNote()),
	}
}

func planRoutes(origin LatLng, originName string, destination *DemoPlace, profile string) ([]RouteOption, string) {
	kinds := layersForProfile(profile)

	dest := LatLng{Lat: destination.Lat, Lng: destination.Lng}
	...
	accessiblePivots := []LatLng{origin}
	...
	fastScore, fastStrokes, fastFacilities, fastBarriers := evaluateRoute(fastLike, destination, profile, []LatLng{origin, dest})
	...
	fastSteps := withWarnings(buildSteps([]LatLng{origin, dest}, destination.Name), fastBarriers, fastFacilities)
...
func decimate(points []LatLng, maxStepM float64) []LatLng {
	if len(points) < 3 {
		return points
	}
	out := []LatLng{points[0]}
...
func parseOsrmCoords(raw interface{}) []LatLng {
	...
	for _, c := range coords {
		...
		if len(out) > 0 && metersBetween(out[len(out)-1], LatLng{Lat: lat, Lng: lng}) < 0.5 {
			continue
		}
...
func fetchStreetLines(origin, destination LatLng) ([][]LatLng, error) {
...
func toStreetOption(
	...
	opt.HonestNote = strPtr("Accessibility-informed street route based on the real OSM road network and available community data.")
	return opt
}

func planStreetRoutes(origin LatLng, originName string, destination *DemoPlace, profile string) streetRouteResult {
	destinationPoint := LatLng{Lat: destination.Lat, Lng: destination.Lng}
