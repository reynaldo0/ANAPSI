...
const SYSTEM_PROMPT = `Kamu adalah BLINDSPOT Voice Chatbot khusus tunanetra. Berbeda dari Asisten deterministik: kamu adalah teman ngobrol empatik, ringkas, audio-first.
Aturan:
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Ambil konteks data BLINDSPOT deterministik (tidak mengarang)
    let context = "";
    try { const ans = askAssistant({ message: msg }); context = `DATA BLINDSPOT: intent=${ans.intent} | ${ans.answerText} Bullets: ${ans.bullets.slice(0,3).join("; ")}`; } catch {}

    const groqKey = process.env.GROQ_API_KEY;
    if (!groqKey) {
      // Fallback deterministik tanpa Groq - tetap berguna
      const ans = askAssistant({ message: msg });
      return ok({ reply: `${ans.answerText} ${ans.bullets[0] ?? ""}`.trim(), source: "deterministic-fallback", context, disclaimer: "Groq belum dikonfigurasi (set GROQ_API_KEY). Jawaban dari data BLINDSPOT deterministik." });
    }
    ...
    return ok({ reply, source: "groq", model: "llama-3.3-70b-versatile" });
