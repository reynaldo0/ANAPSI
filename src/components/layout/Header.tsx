    ...
    <header className="sticky top-0 z-40 border-b border-border/60 glass">
      <nav aria-label="Navigasi utama" className="mx-auto flex h-[60px] w-full max-w-6xl items-center justify-between gap-4 px-4">
        <Link href="/" className="flex shrink-0 items-center gap-2.5">
          <span aria-hidden="true" className="flex h-9 w-9 items-center justify-center rounded-12 primary-solid text-white shadow-card"><Accessibility className="h-5 w-5" /></span>
        ...
        <div className="flex items-center gap-2">
          <Link href="/chatbot" className="hidden items-center gap-1.5 rounded-full primary-solid px-3 py-1.5 text-sm font-black text-white hover:opacity-90 sm:inline-flex"><Sparkles className="h-4 w-4" aria-hidden="true" /> Chatbot</Link>
          ...
          {loading ? null : user ? (
            ...
            <>
              ...
              <Link href="/register" className="rounded-full primary-solid px-4 py-1.5 text-sm font-black text-white">Daftar</Link>
            </>
          )}
        </div>
      </nav>
